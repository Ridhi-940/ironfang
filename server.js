...
cloudinary.config({
    cloud_name: 'dusvscbg3',
    api_key: '121717962455788',
    api_secret: 'rRq5h_TruTGelTacYVGtgDA52U0'
});

let dbconfig = "mysql://avnadmin:AVNS_lXnMqyUHhOLjW5_04c8@mysql-1364a3c1-ridhi-p1.c.aivencloud.com:27625/sports_project";
    ...
    mySqlVen.query("select * from users where emailid=? and password=? ;", [emailid, password], function (err, allRecords) {
        ...
        if (allRecords.length == 1) {
            if (status == 0) {
                ...
                resp.send(allRecords[0].utype); //resp.send(`Valid\n User Type: ${utype}`);
            }
        }
